...
SELECT models.name, brand_name, headquarters FROM models JOIN brands ON (brand_name=brands.name) WHERE models.name = 'Mustang';
...
SELECT brand_name
FROM brands JOIN models on brand_name = brands.name
GROUP BY brand_name
HAVING count(*)>5;
